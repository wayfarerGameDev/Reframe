extends RefCounted
class_name FluxUtilities

# Consts
const PRESETS_DIRECTORY: String = "res://addons/flux/resources/"
...
static func editor_enter(editor_plugin : EditorPlugin) -> void:
	if Engine.is_editor_hint():
		...
		editor_preview_control.custom_minimum_size = Vector2(0,256)
		editor_plugin.add_control_to_bottom_panel(editor_preview_control,"Flux")
...
static func editor_exit(editor_plugin : EditorPlugin) -> void:
	if Engine.is_editor_hint():
		# Preview
		editor_plugin.remove_control_from_bottom_panel(editor_preview_control)
...
static func editor_process(editor_plugin : EditorPlugin, delta: float) -> void:
	if Engine.is_editor_hint():
		...
		# Preview
		var r = fetch_texture_rid(StringName("preview"))
		if r:
			write_texture_rid_to_image(r,editor_preview_image )
			editor_preview_texture_rect.texture = ImageTexture.create_from_image(editor_preview_image)
		# Visible (Presets_
		if selected_nodes.size() > 0 and selected_nodes[0] is FluxEffectNode:
			if editor_button_spatial.get_parent_control() == null:
				...
				editor_preview_control.size.y = 256
		# Hidden (Presets)
		else : if (selected_nodes.size() == 0 or selected_nodes[0] is not FluxEffectNode) and editor_button_spatial.get_parent_control() != null:
				editor_plugin.remove_control_from_container(editor_plugin.CONTAINER_SPATIAL_EDITOR_MENU, editor_button_spatial)
				editor_plugin.remove_control_from_container(editor_plugin.CONTAINER_CANVAS_EDITOR_MENU, editor_button_canvas)
...
static func editor_menu_button_preset_selected(id: int) -> void:
	if Engine.is_editor_hint():
		var selected_node: FluxEffectNode = EditorInterface.get_selection().get_selected_nodes()[0] as FluxEffectNode
		var preset = get_presets()[id - 1] # To offset None option
		#None
		if id == 0:
			selected_node.name = "FluxEffect_None"
			selected_node.uniforms = []
			selected_node.world_alpha = 1
			selected_node.world_resolution = Vector2i(256,256)
			selected_node.functions = ""
			selected_node.main = ""
			selected_node.domain = FluxComputeShader.Domain.COMPOSITOR
			selected_node.target_compositor_mode = FluxEffectNode.TargetCompositorMode.WORLD_ENVIORMENT
		#Preset
		else:
			selected_node.name = "FluxEffect_" + preset.name
...
static func get_presets() -> Array[FluxEffectPreset]:
	var presets: Array[FluxEffectPreset] = []
	...
	if dir:
		...
		while file_name != "":
			...
			if file_name.ends_with(".tres") or file_name.ends_with(".res"):
				...
				if resource is FluxEffectPreset:
					presets.append(resource as FluxEffectPreset)
				else:
