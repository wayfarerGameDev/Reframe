	...
	if domain == Domain.RENDERTARGET and compile_shader():
		...
		for uniform in uniforms:
			if typeof(uniform) == TYPE_STRING_NAME and uniform != "":
				var source_rid = FluxUtilities.create_texture_rid(uniform, world_resolution)
				image_target = FluxUtilities.fetch_texture_rid(uniform)
				dispatch_shader()
