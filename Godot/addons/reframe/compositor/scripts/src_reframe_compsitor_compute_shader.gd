...
void main() 
{
	...
	vec4 color = imageLoad(image_scene_color, index);

	#COMPUTE_MAIN_CODE
	
	// Result
	imageStore(image_scene_color, index, color);
}
"""
...
func compile_shader() -> bool:
	...
	var shader_spirv: RDShaderSPIRV = render_device.shader_compile_spirv_from_source(shader_source)
...
func dispatch_shader_to_compositor(p_effect_callback_type, p_render_data):
	if compile_shader() and domain == Domain.COMPOSITOR:
...
func dispatch_shader_to_render_target():
	if compile_shader():
		for uniform in uniforms:
			if typeof(uniform) == TYPE_STRING and uniform != "" and uniform != "scene":
				ReframeCompositorUtilities.create_texture_rid(uniform, Vector2i(512,512))
...
func dispatch_shader():
	...
	render_device.compute_list_bind_uniform_set(compute_list, UniformSetCacheRD.get_cache(shader, 0, uniforms), 0)
	# World (Push constant)
	...
	render_device.compute_list_set_push_constant(compute_list, world_push_constant.to_byte_array(),  world_push_constant.size() * 4)
	# Run compute shader.
	render_device.compute_list_dispatch(compute_list, x_groups, y_groups, z_groups)
