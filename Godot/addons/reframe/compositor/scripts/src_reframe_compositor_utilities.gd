...
static func editor_enter(editor_plugin : EditorPlugin) -> void:
	if Engine.is_editor_hint():
		...
		editor_plugin.add_control_to_bottom_panel(editor_preview_control,"Compositor")
...
static func editor_process(editor_plugin : EditorPlugin, delta: float) -> void:
	if Engine.is_editor_hint():
		...
		# Preview
		var r = ReframeCompositorUtilities.fetch_texture_rid(StringName("preview"))
		if r:
		...
		# Visible (Presets_
		if selected_nodes.size() > 0 and selected_nodes[0] is ReframeCompsitorEffectNode:
